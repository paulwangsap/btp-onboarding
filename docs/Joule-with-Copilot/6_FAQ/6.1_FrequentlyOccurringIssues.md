...
## First step: run the Joule–Copilot Integration Validation Tool

As an initial step for any of the issues below, run the open-source <a href="https://github.com/microsoft/joule-copilot-integration-validation-tool">Joule–Copilot Integration Validation Tool</a> (Microsoft, Apache-2.0). It exports the **Microsoft Entra ID** and **SAP Cloud Identity Services (IAS)** configuration and lets a Microsoft 365 Copilot agent compare them against the expected Joule setup, producing a report of misconfigurations and recommended fixes. Please follow the instructions in the repository.

> **Note:** The exported JSON contains sensitive identity-provider configuration (tenant IDs, app IDs, trust settings). Only upload it to a Copilot agent in a tenant you control.
